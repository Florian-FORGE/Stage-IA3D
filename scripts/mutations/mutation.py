...
class Mutator():
    ...
    def insert(self, mutation: Mutation, silenced: bool = None):
        """
        Interval will be filled with the specified sequence (mutation.sequence).
        """
        subseq = self.get_ref(mutation)
        mutation.ref = subseq
        if mutation.strand == "+":
    ...
    def permutations_inter(self, mutation: Mutation, binsize: int = None):
        ...
        elif order is not None and len(order) == len(bins) :
            bin_order = order
            new_order_seq = [bins[idx].split("-")[-1] for idx in order]

        else :
            raise ValueError(f"If order ({order}) is given, it sould be coherent with the number of bins in " \
                             f"the interval (binsize : {binsize}, sequence size : {len(subseq)}). Binsize can " \
                             f"be changed in the Config_mut.yaml file.")
    ...
    def permutations_intra(self, mutations: List[Mutation]) :
        ...
        for chrom in l_chrom:
            ...
            seq_order, tries = range(len(chrom_mutations)), 0
            while any([seq_order[i] == i for i in range(len(seq_order))]) and tries <= 10 :
